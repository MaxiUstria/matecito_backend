class IncreaseObjectiveCounterJob < ApplicationJob
  ...
  def perform(user, donation)
    user.objectives.where(state: %w[active goal_reached]).where(
      ...
      increment_current_amount(objective, donation)
  ...
  def increment_current_amount(objective, donation)
    objective.update!(current_amount: objective.current_amount + donation.amount)
